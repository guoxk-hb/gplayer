...
export class GuoPlayer {
  ...
  private init(src: string, type: string) {
    if (type === 'm3u8') this.hls(src);
    else if (type === 'mpd') this.dash(src);
    else if (type === 'flv') this.flv(src);
    else if (type === 'mp4') this.mp4(src);
  ...
  private initByType(src: string, type: string) {
    if (type === 'mpd') this.dash(src);
    else if (type === 'm3u8') this.hls(src);
    else if (type === 'flv') this.flv(src);
    else this.mp4(src);
  }
  ...
  private destroyInternalPlayer() {
    ...
    if (!p) return;
    if (this.state.type === 'mpd') p.reset?.();
    else p.destroy?.();
