...
function parseTimeToSeconds(timeStr: string) {
  ...
  if (timeStr.includes(',')) {
    ...
    return Number.parseInt(parts[0]!) * 3600 + Number.parseInt(parts[1]!) * 60 + Number.parseInt(parts[2]!) + Number.parseInt(parts[3]!) / 1000
  }
  else if (timeStr.includes(':')) {
    parts = timeStr.split(/[:.]/) // "01:30.50"
    return Number.parseInt(parts[0]!) * 60 + Number.parseInt(parts[1]!) + (parts[2] ? Number.parseInt(parts[2]) / 100 : 0)
  }
  return 0
}
